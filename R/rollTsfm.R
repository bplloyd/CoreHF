...
rollTsfm = function(asset.names, factor.names, data, fit.method = "LS", variable.selection = "none", window = 12, fixedWindow = T, on=NULL, par = TRUE){
  ...
  if(par) {
    ...
    parallel::clusterExport(cl = cl, 
                            varlist = c("asset.names", "factor.names", "data", "slices", "fit.method", "variable.selection"),
                            envir = env)
    ...
    res = parallel::parLapply(cl = cl, 
                              X = slices, 
                              fun = function(x)return(factorAnalytics::fitTsfm(asset.names = asset.names[which(colSums(is.na(data[x, asset.names, drop = F]))==0)],
